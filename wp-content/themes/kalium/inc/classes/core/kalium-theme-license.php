	...
	public function admin_init() {
		...
		if ( $this->isEnvatoHostedSite() ) {
			...
			if ( $request_verification ) {
				...
				$url = remove_query_arg( 'purchase_code_request_verification' );
				wp_redirect( $url );
			}

			// Verification errors
			if ( $license_errors = get_theme_mod( 'purchase_code_request_verification_error' ) ) {
				if ( 'remove' === kalium()->url->get( 'purchase_code_request_verification_error' ) ) {
					remove_theme_mod( 'purchase_code_request_verification_error' );
					wp_redirect( remove_query_arg( 'purchase_code_request_verification_error' ) );
				}

				$dismiss_link   = sprintf( '<a href="%s" class="dismiss-link">Dismiss</a>', esc_url( add_query_arg( 'purchase_code_request_verification_error', 'remove' ) ) );
				$license_errors = sprintf( 'An error occurred during product activation: <strong>%s</strong> %s', esc_html( $license_errors ), $dismiss_link );
